              ...
              >
                ...
                <td style={tdStyle}>
                  <span style={{ color: '#9ca3af', fontSize: '13px' }}>
                    {team.description || <span style={{ color: '#4b5563', fontStyle: 'italic' }}>No description</span>}
                  </span>
                </td>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <p style={{ color: '#94a3b8', fontSize: '13px', marginTop: '4px' }}>
                    {viewTeam.description || 'No description'}
                  </p>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                ...
                <div>
                  ...
                  <p style={{ color: '#e5e7eb', fontSize: '13px', fontWeight: '600', margin: 0 }}>
                    {viewTeam.created_by_detail?.name || 'Unknown'}
                  </p>
                </div>
              </div>
                    ...
                    <div key={i} style={{
                      ...
                      {m.role && (
                        <span style={{
                          ...
                          padding: '3px 10px', borderRadius: '999px', whiteSpace: 'nowrap', flexShrink: 0, marginLeft: '8px',
                        }}>{m.role}</span>
                      )}
                      ...
                      {canEdit && (
                        <button
                          ...
                          style={{
                            ...
                            padding: '3px 10px', borderRadius: '8px', cursor: removingId === m.id ? 'not-allowed' : 'pointer',
                            whiteSpace: 'nowrap', flexShrink: 0, marginLeft: '8px',
                            opacity: removingId === m.id ? 0.6 : 1,
                          }}
                          onMouseEnter={e => { if (removingId !== m.id) { e.currentTarget.style.background = 'rgba(239,68,68,0.15)' } }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}
                        >
...
function EditTeamModal({ team, onClose, onSaved }) {
  ...
  const handleSaveRole = async (memberRowId) => {
    setSavingRoleId(memberRowId)
    try {
      const member = members.find((m) => m.id === memberRowId)
      await api.put(`/team-members/${memberRowId}/`, {
        team: team.id,
        user: member.user,
        role: roleDrafts[memberRowId],
      })
      setMembers((prev) =>
        prev.map((m) => (m.id === memberRowId ? { ...m, role: roleDrafts[memberRowId] } : m))
      )
    } catch (err) {
      console.error('Failed to update member role', err)
      alert('Could not update this member\'s role.')
    } finally {
  ...
  return (
    <div style={overlayStyle}>
      <div style={{ ...modalStyle, maxWidth: '840px' }}>
        <div style={headerStyle}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {members.length === 0 ? (
              <p style={{ color: '#6b7280', fontSize: '13px', marginTop: '8px' }}>No members yet.</p>
            ) : (
                  ...
                  <div key={m.id} style={{
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      <button
                        ...
                        disabled={savingRoleId === m.id || roleDrafts[m.id] === m.role}
                        style={{
                          background: '#2d3348', border: 'none', color: '#a5b4fc',
                          fontSize: '11px', fontWeight: '600', padding: '6px 10px',
                          borderRadius: '8px', cursor: (savingRoleId === m.id || roleDrafts[m.id] === m.role) ? 'not-allowed' : 'pointer',
                          opacity: (savingRoleId === m.id || roleDrafts[m.id] === m.role) ? 0.5 : 1,
                          whiteSpace: 'nowrap',
                        }}
                      >
                        {savingRoleId === m.id ? '...' : 'Save'}
                      </button>
                      <button
                        onClick={() => handleRemoveMember(m.id)}
                        disabled={removingId === m.id}
                        style={{
                          ...
                          opacity: removingId === m.id ? 0.6 : 1, whiteSpace: 'nowrap',
                        }}
                        onMouseEnter={e => { if (removingId !== m.id) e.currentTarget.style.background = 'rgba(239,68,68,0.15)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}
                      >
          ...
          <div style={{ padding: '28px 24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', paddingTop: '20px', borderTop: '1px solid #2d3348' }}>
              ...
              >Close</button>
              <button onClick={handleSaveTeamFields} disabled={saving}
              ...
              >{saving ? 'Saving...' : 'Save Team Details →'}</button>
            </div>
          </div>
